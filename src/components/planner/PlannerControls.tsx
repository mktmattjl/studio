  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4 sm:mb-6">
      <div className="flex items-center gap-2">
        <Button onClick={onPrev} variant="outline" size="icon" aria-label={`Previous ${viewMode}`}>
          <ChevronLeft className="w-5 h-5" />
        </Button>
        <Button onClick={onNext} variant="outline" size="icon" aria-label={`Next ${viewMode}`}>
          <ChevronRight className="w-5 h-5" />
        </Button>
        <Button onClick={onToday} variant="outline" size="default">
          Today
        </Button>
      </div>
